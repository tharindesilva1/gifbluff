...
export const Home: FunctionComponent<HomeProps> = ({
    ...
    const joinAllowed = !loading && validatePlayerId(playerId) && (homeState === HomeState.CREATE || validateSessionId(sessionId));
    ...
    const joinSession = async () => {
        ...
        try {
            ...
            const callback = (success: boolean) => {
                if (success) {
                    const sessionInfo: ISessionInfo = { sessionId: joiningSessionId, playerId: playerId.trim() };
                    saveSessionInfo(sessionInfo)
                    history.push("/session");
                }
                else setError(true);
            }
            await serverWorker.openWebsocket(joiningSessionId, playerId, proxy(callback));
        ...
        return (
            <div className={styles.home}>
                <Panel className={styles.form}>
                    ...
                    {!creatingGame && <motion.label {...motionProps}>{"Room code:"}</motion.label>}
                    {!creatingGame && <motion.input {...motionProps} onChange={setSessionId} value={sessionId} placeholder={"5 character code"}></motion.input>}
                    {<motion.label {...motionProps}>{"Your name:"}</motion.label>}
                    <motion.input {...motionProps} onChange={setPlayerId} value={playerId} placeholder={"Up to 12 character"}></motion.input>
                    {error && <motion.small {...motionProps}>
    ...
    return (
        homeState === HomeState.ROOT ? renderRoot() : renderForm()
    )
}