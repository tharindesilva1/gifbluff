...
const renderGifs = (
  ...
  return (
    <section
      ref={sectionRef}
      className={[styles.gifPreview, !searchString && styles.hidden]
        .filter(Boolean)
        .join(" ")}
    >
      {!!(urls[0] && urls[0].length && searchString === searchedGifString) &&
        renderColumns()}
      {!searchString && renderGifSearchHelper()}
      {searching && searchString !== searchedGifString && (
        <LoadingIndicator className={styles.loading} />
      )}
      {!searching && urls[0] && !urls[0].length && (
...
const GifSelector: ForwardRefRenderFunction<
  HTMLDivElement,
  GifSelectorProps
> = (
  {
  ...
  const latestValsRef = useRef<{
    onScrollDown: (str: string) => void;
    searchString: string;
    searchedGifString: string;
  }>();
      ...
      if (sectionRef.current && latestValsRef.current) {
        ...
        if (searchString && searchString === searchedString)
          if (isScrolledToBottom()) {
            latestValsRef.current.onScrollDown(
              latestValsRef.current.searchString
            );
          }
      }
    });
  ...
  useEffect(() => {
    if (
      ...
      searchString &&
      searchString === searchedGifString
    ) {
      if (isScrolledToBottom()) {
  ...
  return (
    <section
      ref={ref}
      className={[styles.gifSelector, className].filter(Boolean).join(" ")}
    >
      {renderGifs(
        gifUrls,
        searchString,
...
export default motion(
  React.forwardRef<HTMLDivElement, GifSelectorProps>(GifSelector)
);
